...
const Register = (): React.JSX.Element => {
  ...
  try {
    const res = await fetch('http://192.168.0.13:5001/register-face', {
      method: 'POST',
      body: formData,
    });
    ...
    console.error('Fetch error during sendPhotos:', fetchError);
    if (fetchError instanceof TypeError && fetchError.message === 'Network request failed') {
      console.error('NETWORK ERROR: Could not reach the server. Please check if your phone is on the same Wi-Fi as the server and the IP address (192.168.0.13) is correct.');
    }
    throw fetchError;
  }
};
...
const captureFaceData = async () => {
  try {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (photo?.base64) {
        console.log(`Picture ${i + 1} taken, base64 length: ${photo.base64.length}`);
        photos.push({ base64: photo.base64 });
      } else {
        console.warn(`Picture ${i + 1} failed: base64 is missing`);
      }
    ...
    console.log(`All 5 pictures taken. Sending to server...`);
    ...
    console.log('Face data captured and sent successfully');
    Alert.alert('Face data captured successfully!', 'You can now complete registration.');
  } catch (err) {
    console.error('Error in captureFaceData details:', err);
    if (err instanceof Error) {
      setError('Error capturing face data: ' + err.message);
    } else {
      setError('Error capturing face data: ' + String(err));
    }
  } finally {
    setCapturing(false);
    setCameraVisible(false);
    setPicturesLeft(0);
    console.log('=== captureFaceData finished ===');
  }
};
  ...
  const handleRegister = async () => {
    ...
    try {
      const res = await fetch('http://192.168.0.13:3001/users', {
        method: 'POST',
